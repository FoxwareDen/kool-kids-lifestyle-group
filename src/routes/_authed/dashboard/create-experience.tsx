...
type SkeletonPageData = Omit<
  BookingPage,
  'blocks' | 'createdAt' | 'updatedAt' | 'id' | 'slug'
>
...
function useCategories(
  ...
  const remove = useCallback(
    (val: string) => {
      setPageData((prev) => ({
        ...prev,
        category: serializeCategories(
          parseCategories(prev.category).filter((c) => c !== val),
        ),
      }))
    },
    [setPageData],
...
function HeaderBlockEditor({
  ...
  block: Extract<PageBlock, { type: 'header' }>
  lang: Language
  onChange: (b: Extract<PageBlock, { type: 'header' }>) => void
}) {
  return (
...
function ParagraphBlockEditor({
  ...
  lang: Language
  onChange: (b: Extract<PageBlock, { type: 'paragraph' }>) => void
}) {
  return (
    <label className="block">
      <span className="mb-1 block text-sm font-semibold text-[var(--sea-ink)]">
...
function ImageBlockEditor({
  ...
  lang: Language
  onChange: (b: Extract<PageBlock, { type: 'image' }>) => void
}) {
      ...
      <label className="block">
        ...
        {previewUrl && !error && (
          <img
            ...
            src={previewUrl}
            alt="preview"
            className="mt-1 rounded-sm max-h-36 object-cover w-full"
          />
        )}
      </label>
      <label className="block">
        ...
        <input
          ...
          onChange={(e) =>
            onChange({
              ...block,
              alt: setTranslated(block.alt, lang, e.target.value),
            })
          }
        />
      </label>
      <label className="block">
        ...
        <input
          ...
          onChange={(e) =>
            onChange({
              ...block,
              caption: setTranslated(
                block.caption ?? { default: '' },
                lang,
                e.target.value,
              ),
            })
...
export function MediaBlockEditor({
  ...
  block: Extract<PageBlock, { type: 'media' }>
  lang: Language
  onChange: (b: Extract<PageBlock, { type: string }>) => void
}) {
  ...
  return (
    <div className="flex flex-row gap-3">
      <label className="block">
        <span className="mb-1 block text-sm font-semibold text-[var(--sea-ink)] capitalize">
          {mode}
        </span>
        <MediaModel
          open={open}
          toggleOpen={toggleModel}
          onClick={bindOnChange}
          accept={mode}
        />
      ...
      <label className="block capitalize">
        <SelectField
          ...
          value={mode}
          onChange={(e) =>
            setMode(e.target.value as MediaBlockEditorAcceptedTypes)
          }
          className="w-32"
        >
...
function VideoBlockEditor({
  ...
  lang: Language
  onChange: (b: Extract<PageBlock, { type: 'video' }>) => void
}) {
  const previewUrl = useObjectUrl(block.file)
  const [error, setError] = useState<string | null>(null)
  return (
      ...
      <label className="block">
        ...
        {previewUrl && !error && (
          <video
            src={previewUrl}
            controls
            className="mt-1 rounded-sm max-h-36 w-full"
          />
        )}
      </label>
      <label className="block">
    ...
    <div className="flex flex-col gap-3 rounded-sm border border-[var(--line)] bg-[var(--surface-strong)] p-3.5">
      <div className="flex items-center justify-between border-b border-[var(--line)] pb-2.5">
        ...
        <button
          type="button"
          onClick={onDelete}
          className="text-[var(--sea-ink-soft)] hover:text-[var(--destructive)]"
        >
          <Trash2 className="size-3.5" />
        </button>
      </div>
      {block.type === 'header' && (
        <HeaderBlockEditor block={block} lang={lang} onChange={onChange} />
      )}
      {block.type === 'paragraph' && (
        <ParagraphBlockEditor block={block} lang={lang} onChange={onChange} />
      )}
      {block.type === 'media' && (
        <MediaBlockEditor block={block} lang={lang} onChange={onChange} />
      )}
      {block.type === 'image' && (
        <ImageBlockEditor block={block} lang={lang} onChange={onChange} />
      )}
      {block.type === 'video' && (
        <VideoBlockEditor block={block} lang={lang} onChange={onChange} />
      )}
    </div>
  )
}
...
function RouteComponent() {
  ...
  const { entries, blocks, addBlock, updateBlock, deleteBlock, serialize } =
    useBlocks()
      ...
      <div className="flex-1 flex flex-col gap-5 overflow-y-auto bg-[var(--surface-strong)] p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-1 text-2xl font-bold text-[var(--sea-ink)]">
              Create Experience
            </h1>
          </div>
          <SelectField
            ...
            <label className="block">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!file) return
                  if (file.size > MAX_SIZE) {
                    setSubmitError('Cover image exceeds 5MB.')
                    e.target.value = ''
                    return
                  }
                  setSubmitError(null)
              ...
              {categories.categories.length > 0 && (
                <div className="mt-1.5 flex flex-wrap gap-1.5">
                  {categories.categories.map((c) => (
                    <span
                      key={c}
                      className="inline-flex items-center gap-1 rounded-sm bg-[color-mix(in_oklab,var(--brand-orange)_16%,transparent)] py-0.5 pl-2.5 pr-1.5 text-xs font-semibold text-[var(--brand-orange-deep)]"
                    >
                      {c}
                      <button
                        type="button"
                        onClick={() => categories.remove(c)}
                        className="leading-none text-[var(--brand-orange-deep)]/60 hover:text-[var(--destructive)]"
                      >
                        ✕
                      </button>
                    </span>
            ...
            <div className="flex flex-wrap gap-2">
              {BLOCK_TYPES.map((type) => (
                <button
                  ...
                  className="rounded-sm border border-[var(--line)] px-2.5 py-1 text-xs font-semibold text-[var(--sea-ink-soft)] transition-colors hover:border-[var(--brand-orange)] hover:text-[var(--brand-orange)]"
                >
                  + {type}
                </button>
              ))}
        ...
        {submitError && (
          <p className="rounded-sm bg-[color-mix(in_oklab,var(--destructive)_10%,transparent)] px-3 py-2 text-sm text-[var(--destructive)]">
            {submitError}
          </p>
        )}
