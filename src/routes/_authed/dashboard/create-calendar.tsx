...
const fetchUnits = createServerFn().handler(async () => {
...
function RouteComponent() {
  ...
  const isAllDay = useMemo(
    () => calendarForm.booking_type == 'day',
    [calendarForm.booking_type],
  )
  ...
  const [bufferUnit, setBufferUnit] = useState<'minutes' | 'hours' | 'days'>(
    'minutes',
  )
  ...
  useEffect(() => {
    if (calId && isLoaderReady) {
      ;(async () => {
        const data = (location.state as unknown as { calendar: Calendar })
          ?.calendar
  ...
  function updateField<K extends keyof typeof calendarForm>(
    key: K,
    value: (typeof calendarForm)[K],
  ) {
    ...
    <div className="mx-auto max-w-4xl px-6 py-8">
      ...
      <div className="space-y-6">
        ...
        >
          ...
          {experienceLoading && (
            <p className="mt-4 italic text-[var(--sea-ink-soft)]">
                  ...
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger className="shrink-0 text-[var(--sea-ink-soft)]">
                        <span className="flex size-5 items-center justify-center rounded-full border border-[var(--line)] text-xs">
                          ?
                        </span>
                      </TooltipTrigger>
              ...
              {!isAllDay && (
                <div>
                  ...
                  <p className="mb-2 text-xs text-[var(--sea-ink-soft)]">
                    Time blocked off before the next booking can start — e.g.
                    cleanup for a room, or a gap between events.
                  </p>
                  <div className="flex gap-2">
                    <input
                      ...
                      value={bufferAmount}
                      onChange={(e) =>
                        updateBuffer(Number(e.target.value), bufferUnit)
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateBuffer(
                          bufferAmount,
                          e.target.value as typeof bufferUnit,
                        )
                      }
                    ...
                    <div className="flex w-full items-end gap-2">
                      <div className="grow">
                        <TextField
                          ...
                          value={newUnit.label}
                          onChange={(e) =>
                            setNewUnit((p) => ({ ...p, label: e.target.value }))
                          }
                        />
                      </div>
                      <Button
