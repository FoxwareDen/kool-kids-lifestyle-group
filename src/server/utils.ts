...
export const initializePayment = createServerFn()
  ...
  .handler(
    async ({
      data: { amount, email, phone, name, code, reference },
      ...
      const testEmail = domain
        ? `${username}+test_${Date.now()}@${domain}`
      ...
      if (!createCustomerRes.ok) {
        await fetch(
          `https://api.paystack.co/customer/${encodeURIComponent(testEmail)}`,
          {
            method: 'PUT',
            headers,
            body: JSON.stringify({
