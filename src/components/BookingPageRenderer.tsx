    ...
    <figure className="flex flex-col gap-2">
      {src ? (
        <img
          ...
          className="w-full mx-auto h-96 object-cover rounded-xl shadow-lg shadow-[var(--brand-navy)]/10"
        />
      ) : (
    ...
    <div className="mx-auto flex max-w-2xl flex-col gap-6 px-6 py-10 md:px-10 md:py-14">
      {page.blocks.map((block) => {
        switch (block?.type) {
          case 'header':
            return (
              <HeaderRenderer key={block.index} block={block} lang={lang} />
            )
          case 'paragraph':
            return (
              <ParagraphRenderer key={block.index} block={block} lang={lang} />
            )
          case 'image':
          ...
          default:
            return (
              <div key={(block as any).index}>Oops component not found</div>
            )
        }
      })}
    </div>
...
export const HydratedHeaderRenderer = ({
  block,
  lang,
}: HydratedHeaderRendererProps) => {
  const text = resolveTranslatable(block.text, lang)
...
export const HydratedParagraphRenderer = ({
  block,
  lang,
}: HydratedParagraphRendererProps) => {
  const text = resolveTranslatable(block.text, lang)
...
export const HydratedImageRenderer = ({
  block,
  lang,
}: HydratedImageRendererProps) => {
...
export const HydratedVideoRenderer = ({
  block,
  lang,
}: HydratedVideoRendererProps) => {
