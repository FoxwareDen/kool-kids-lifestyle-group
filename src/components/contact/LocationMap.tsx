...
export function LocationMap({ lang = 'en' }: { lang?: Language }) {
  const heading = resolveTranslatable(
    { default: 'Find Us', translations: { af: 'Vind Ons' } },
    lang,
  )
  const title = resolveTranslatable(
    {
      default: 'Where to Find Prieska',
      translations: { af: 'Waar om Prieska te Vind' },
    },
    lang,
  )
  const cta = resolveTranslatable(
    {
      default: 'View on Google Maps',
      translations: { af: 'Beskou op Google Maps' },
    },
    lang,
  )
  const place = resolveTranslatable(
    {
      default: 'Prieska, Northern Cape',
      translations: { af: 'Prieska, Noord-Kaap' },
    },
    lang,
  )
  const addressLine1 = resolveTranslatable(
    {
      default: ' 18 Steward Street, Prieska, 8940',
      translations: { af: 'Victoriaweg, Prieska, 8940' },
    },
    lang,
  )
  ...
  return (
    <section className="bg-white py-20">
      <div className="mx-auto w-full max-w-[1180px] px-4 sm:px-6 lg:px-8">
        <SectionHeading eyebrow={heading} title={title} theme="light" />
