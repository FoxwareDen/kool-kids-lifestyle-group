...
export const useBookerStore = create<BookerState>((set) => ({
  ...
  filterByUnit: (unit: Unit) =>
    set((prev) => ({
      ...prev,
      filteredSchedules: prev.schedule.filter((sc) =>
        sc.units.flatMap((u) => u.id).includes(unit.id),
      ),
    })),
  setSelectedSchedule: () => {},
...
export function BookerStep({
  ...
  return (
    <div
      className={cn(
        'w-full min-w-0 flex justify-center items-center',
        className,
      )}
    >
...
export function BookingCalendar({ className }: { className?: string }) {
  ...
  const availableDates: { from: Date; to: Date }[] = filteredSchedules.flatMap(
    (r) => ({
  ...
  const handleChange = (date: Date) => {
    ...
    const range = filteredSchedules.find(
      (fs) => formatted >= fs.start_date && formatted <= fs.end_date,
    )
    if (!range) return
    setBooking({
      ...booking,
  ...
  const handleRangeChange = (range: DateRange | undefined) => {
    ...
    const spansDisabled = days.some(
      (day) =>
        !availableDates.some((t) =>
          isWithinInterval(day, { start: t.from, end: t.to }),
        ),
    )
    ...
    const matchingRange = filteredSchedules.find(
      (fs) => fromFormatted >= fs.start_date && fromFormatted <= fs.end_date,
    )
  ...
  if (type === 'slot') {
    return (
      <Calendar
        ...
        disabled={(date: Date) =>
          !availableDates.some((t) =>
            isWithinInterval(date, { start: t.from, end: t.to }),
          )
        }
      />
    )
...
function parseTimeToMinutes(
  ...
  if (
    !isAM &&
    !isPM &&
    referenceStartMin !== undefined &&
    totalMinutes < referenceStartMin
  ) {
...
export function BookingTimeSelect({ className }: { className?: string }) {
  const { date, filteredSchedules, selectedUnit, booking, setBooking } =
    useBookerStore()
  ...
  const activeRanges = useMemo(() => {
    ...
    return filteredSchedules
      .filter(
        (fs) => formattedDate >= fs.start_date && formattedDate <= fs.end_date,
      )
      .map((fs) => {
  ...
  const slots = useMemo(() => {
    ...
    const slotDuration =
      selectedUnit?.duration && selectedUnit.duration > 0
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-xs text-muted-foreground">
                {selectedUnit?.duration
                  ? `${selectedUnit.duration}-minute slots`
                  : '30-minute interval timeline'}
              </CardDescription>
            </div>
            {booking.start_time && (
              <Badge
                variant="secondary"
                className="gap-1 px-2.5 py-1 text-xs font-mono"
              >
          ...
          <>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium">
                {booking.date
                  ? format(
                      addDays(parseISO(booking.date), booking.duration),
                      'EEE, MMM d yyyy',
                    )
                  : '-'}
              </span>
...
export function BookingPagingButtonGroup({
  ...
  return (
    <div
      className={cn(
        'flex items-center justify-between gap-3 pt-3 border-t w-full',
        className,
      )}
    >
      <Button
