...
export const BLOCK_TYPES: PageBlock['type'][] = [
  'header',
  'paragraph',
  'image',
  'video',
  ...
  <div className="flex flex-col gap-3 rounded-lg border border-[var(--brand-navy)]/15 bg-white p-3.5 shadow-sm shadow-[var(--brand-navy)]/5">
    <div className="flex items-center justify-between border-b border-[var(--brand-navy)]/10 pb-2.5">
      ...
      <button
        type="button"
        onClick={onDelete}
        className="text-xs text-[var(--brand-navy)]/30 hover:text-[var(--brand-orange)]"
      >
        ✕
      </button>
    </div>
    ...
    {block.type === 'selectable' && (
      <SelectableBlockEditor block={block} lang={lang} onChange={onChange} />
    )}
  </div>
)
