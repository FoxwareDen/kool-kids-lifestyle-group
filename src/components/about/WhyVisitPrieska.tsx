...
const REASONS: { icon: string; title: string; description: string }[] = [
  {
    ...
    description:
      "Boating, fishing and riverside calm along South Africa's longest river.",
  },
  {
    icon: 'Mountain',
    title: 'Open Karoo Landscapes',
    description:
      'Endless plains, dramatic koppies and some of the clearest night skies anywhere.',
  },
  {
    icon: 'Landmark',
    title: 'Living Heritage',
    description:
      'Historic landmarks and stories that span generations of the Northern Cape.',
  },
  {
    icon: 'Tent',
    title: 'Adventure & Recreation',
    description:
      'Hiking, cycling, quad trails and guided outdoor experiences for every pace.',
  },
  {
    icon: 'Sun',
    title: 'Unforgettable Sunsets',
    description:
      "Golden-hour views that turn the Karoo into a photographer's dream.",
  },
  {
    icon: 'HandHeart',
    title: 'Genuine Hospitality',
    description:
      'Warm, community-driven service that makes every visitor feel at home.',
  },
]
...
export function WhyVisitPrieska({
  ...
  data: Content<Partial<WhyVisitPrieska>>
  lang?: Language
}) {
  const {
    content: { kicker, title, reasons },
  } = data
  const defaultKicker = resolveTranslatable(
    {
      default: 'A Destination Like No Other',
      translations: { af: 'n Bestemming soos Geene Ander' },
    },
    lang,
  )
  const defaultTitle = resolveTranslatable(
    {
      default: 'Why Visit Prieska',
      translations: { af: 'Waarom Prieska Besoek?' },
    },
    lang,
  )
