...
function booking(
  overrides: Partial<Booking> &
    Pick<Booking, 'date' | 'start_time' | 'end_time'>,
): Booking {
  return {
...
function dayBooking(
  overrides: Partial<Booking> & Pick<Booking, 'date' | 'duration'>,
): Booking {
  return {
    id: 'bk_' + Math.random().toString(36).slice(2),
    start_time: '14:00',
...
describe('slot mode – no bookings', () => {
  it('returns one full-day range per operating day', () => {
    const result = generateAvailableSlots(slotCalendar(), noBookings, {
      minAdvanceDays: 0,
    })
...
describe('slot mode – single booking', () => {
  it('splits the day into two windows around a mid-day booking', () => {
    ...
    const result = generateAvailableSlots(slotCalendar(), [bk], {
      minAdvanceDays: 0,
    })
    ...
    const [before, after] = monday.sort((a, b) =>
...
describe('slot mode – multiple bookings on the same day', () => {
  it('carves multiple windows from multiple bookings', () => {
    ...
    const result = generateAvailableSlots(slotCalendar(), [bk1, bk2], {
      minAdvanceDays: 0,
    })
  ...
  it('handles three bookings leaving two gaps', () => {
    ...
    const result = generateAvailableSlots(slotCalendar(), [bk1, bk2, bk3], {
      minAdvanceDays: 0,
    })
...
describe('slot mode – multiple units', () => {
  it('tracks availability independently per unit', () => {
    ...
    const mondayWithB = result.filter(
      (r) =>
        r.start_date === '2026-08-03' &&
        r.units.some((u) => u.id === UNIT_B.id),
    )
...
describe('slot mode – days of week filtering', () => {
  it('excludes dates that fall on non-operating days', () => {
    ...
    const result = generateAvailableSlots(cal, noBookings, {
      minAdvanceDays: 0,
    })
...
describe('day mode – no bookings', () => {
  it('returns a single contiguous block spanning the full horizon', () => {
    const result = generateAvailableSlots(dayCalendar(), noBookings, {
      minAdvanceDays: 0,
    })
...
describe('day mode – single booking', () => {
  it('splits the horizon into two blocks around a booked stay', () => {
    ...
    const result = generateAvailableSlots(dayCalendar(), [bk], {
      minAdvanceDays: 0,
    })
  ...
  it('blocks the booked day itself (Aug 5 with duration 1)', () => {
    ...
    const rangesContainingAug5 = result.filter(
      (r) =>
        r.start_date <= '2026-08-05' &&
        (r.end_date ?? r.start_date) >= '2026-08-05',
    )
...
describe('day mode – multiple bookings', () => {
  it('produces three ranges around two non-adjacent bookings', () => {
    ...
    const result = generateAvailableSlots(dayCalendar(), [bk1, bk2], {
      minAdvanceDays: 0,
    })
  ...
  it('adjacent bookings leave no gap between them', () => {
    ...
    const rangesContainingAug6 = result.filter(
      (r) =>
        r.start_date <= '2026-08-06' &&
        (r.end_date ?? r.start_date) >= '2026-08-06',
    )
...
describe('day mode – buffer', () => {
  it('enforces buffer_minutes between consecutive bookings on the same unit', () => {
    ...
    const result = generateAvailableSlots(cal, [bk1, bk2], {
      minAdvanceDays: 0,
    })
...
describe('day mode – multiple units', () => {
  it('tracks availability independently per unit', () => {
    ...
    const bkA = dayBooking({
      date: '2026-08-05',
      duration: 2,
      unit_id: UNIT_A.id,
      unit_label: UNIT_A.label,
    })
    const bkB = dayBooking({
      date: '2026-08-09',
      duration: 2,
      unit_id: UNIT_B.id,
      unit_label: UNIT_B.label,
    })
    const result = generateAvailableSlots(cal, [bkA, bkB], {
      minAdvanceDays: 0,
    })

    const aRanges = result.filter((r) =>
      r.units.some((u) => u.id === UNIT_A.id),
    )
    const bRanges = result.filter((r) =>
      r.units.some((u) => u.id === UNIT_B.id),
    )
...
describe('edge cases', () => {
  ...
  it('rescheduled bookings are treated as active (not available)', () => {
    ...
    const monday = result.filter((r) => r.start_date === '2026-08-03')
    expect(
      monday.some((r) => r.start_time === '09:00' && r.end_time === '17:00'),
    ).toBe(false)
  })
  ...
  it('booking on a non-operating day does not affect operating days', () => {
    ...
    const result = generateAvailableSlots(cal, [bk], { minAdvanceDays: 0 })
    const operatingDays = result.filter(
      (r) => r.start_date >= '2026-08-10' && r.start_date <= '2026-08-14',
    )
