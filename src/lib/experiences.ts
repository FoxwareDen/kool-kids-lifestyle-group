...
export type HydratedImageBlock = Omit<
  StorageMediaBlock,
  'asset_id' | 'asset_collectionId' | 'asset_file'
> & {
  type: 'image'
  url: string
}

export type HydratedVideoBlock = Omit<
  StorageMediaBlock,
  'asset_id' | 'asset_collectionId' | 'asset_file'
> & {
  type: 'video'
...
export function hydrateBlocks(blocks: StoragePageBlock[]): HydratedPageBlock[] {
  ...
  return blocks.map((block) => {
    if (block.type === 'image' || block.type === 'video') {
      const { asset_collectionId, asset_id, asset_file, ...rest } =
        block as StorageMediaBlock
      return {
        ...rest,
...
// ============================================================
export type CreateBookingPageInput = Omit<
  BookingPage,
  'id' | 'createdAt' | 'updatedAt'
>
export type UpdateBookingPageInput = Omit<
  BookingPage,
  'id' | 'createdAt' | 'updatedAt'
>
...
export async function createBookingPage(
  ...
  try {
    ...
    const storageBlocks: StoragePageBlock[] = blockUploadResults.map(
      (blockUploadResult) => {
            ...
            return {
              ...
              alt:
                originalBlock.type === 'image'
                  ? (originalBlock as ImageBlock).alt
                  : undefined,
              caption:
                originalBlock.type === 'image'
                  ? (originalBlock as ImageBlock).caption
                  : undefined,
              title:
                originalBlock.type === 'video'
                  ? (originalBlock as VideoBlock).title
                  : undefined,
...
export type FeatureCard = Omit<
  FlatBookingPage,
  'expand' | 'blocks' | 'slug' | 'coverImage'
> & {
  coverImage: string
  lang: Language
}

export async function fetchFeaturedExperienceCard(
  lang: Language = 'en',
  cookieHeader?: string,
): Promise<Result<FeatureCard[], string>> {
  let client
...
export async function fetchAllExperiencesCard(
  lang: Language = 'en',
  cookieHeader?: string,
): Promise<Result<FeatureCard[], string>> {
  let client
...
export async function hydrateBlocksAsync(
  ...
  const mediaBlocks = blocks.filter(
    (b) => (b.type === 'image' || b.type === 'video') && b.asset_id,
  )
  ...
  if (assetIds.length > 0) {
    try {
      ...
      const assetRecords = await client
  ...
  return blocks.map((block) => {
    if (block.type === 'image' || block.type === 'video') {
      ...
      return {
        ...rest,
        url: fileName
          ? buildImageUrl(collectionId, block.asset_id, fileName)
          : '',
      } as HydratedPageBlock
    }
    return block as HydratedPageBlock
...
export async function fetchExperienceById(id: string, cookieHeader?: string) {
  ...
  try {
    const record: FlatBookingPage | null = await client
      .collection('Experiences')
      .getOne(id, {
        expand: 'coverImage',
      })
...
// UTILITIES
// ============================================================
export function resolveTranslatable<T>(
  field: Translatable<T>,
  lang: Language,
): T {
...
export function experienceHasCategory(
  experience: { category: string },
  category: string,
): boolean {
  const target = category.toLowerCase()
  return parseCategories(experience.category).some(
